...
def nearest_two(x):
    ...
    power_of_two = 1.0
    if x > 2:
        return 2.0 * nearest_two(x / 2)
    elif x < 1:
        return 1/2 * nearest_two(x * 2)
    elif (x - power_of_two) >= (2 - x):
        return 2.0
    return power_of_two
...
def smooth(f, dx):
    ...
    0.667
    """
    return lambda x :1/3 * (f(x+dx) + f(x) + f(x-dx))
...
def near_golden(perimeter):
    ...
    """
    h, selection = 1, {}
    while h <= (perimeter // 4) + 1:
        w = 1/2 * perimeter - h
        deviation = abs(h * h - w * w + h * w)
        selection.setdefault(h, deviation)
        h = h + 1
    minvalue = min([selection[h] for h in selection])
    for key in selection:
        if selection[key] == minvalue:
            return key
    #without using the min or max function because that it
    #will return an interger instead of a char
...
def intersection(xs, ys):
    ...
    if len(xs) > len(ys):
        ...
        llst, slst = ys, xs
    if slst == llst.rest or slst == llst:
        return slst
    else:
        return intersection(slst, llst.rest)
