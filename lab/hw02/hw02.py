...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    if pred == true:
        ...
        return reduce(combiner,[term(s) for s in range(1,n+1) if odd(s)],base)
...
def g_iter(n):
    ...
    True
    """
    i,total1,total2,total3 = 3 , 3 , 2 , 1
    k, p = 0 , 0
    total = 0
    if n <= 3:
        return n
    while i < n:
        ...
        k, p = total1, total2
        total1,total2,total3 = total,k,p
        i = i + 1
    return total
...
def pingpong(n):
    ...
    def recursion(n,m):
        if n == 1:
            return 1
        if is_odd(m):
            return recursion(n-1,count_j(n-2)) - 1
        else:
            return recursion(n-1,count_j(n-2)) + 1
    return recursion(n,count_j(n-1))
...
def count_change(amount):
    ...
    9828
    """
    def the_largest(amount):
        i = 0
        while amount // 2 != 0:
                amount = amount // 2
                i = i + 1
        return i # seperate the Question in dichotomous cases
    k = pow(2,the_largest(amount))
    def count_change_dynamic(amount,k):
            if k == 1:
                return 1
            elif amount < 0:
                return 0
                ...
                return count_change_dynamic(amount - k, k)+\
                count_change_dynamic(amount,k//2)
    return count_change_dynamic(amount,k)
...
def Y_tester():
    ...
    """
    "*** YOUR CODE HERE ***"
    return Y(lambda n: 1 if n == 1 else n)  # Replace
