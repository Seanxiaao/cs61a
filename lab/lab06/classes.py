...
class Player(object):
    ...
    def go_to(self, location):
        ...
        if destination_place.locked:
            ...
            print("You are at {}".format(destination_place.name))
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            if thing in self.place.things:
                ...
                if 'Key' not in self.place.things[thing].name:
                    self.backpack.append(Thing(self.place.things[thing].name,
                    self.place.things[thing].description))#re-define the class
                else:
                    self.backpack.append(Key(self.place.things[thing].name,
                    self.place.things[thing].description))
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        if self.place.exits[place][0].locked:
            if type(key) == Key:
                print("{} is now unlocked!".format(place))
                self.place.exits[place][0].locked = False
            else:
                print("{} can't be unlocked without a key!".format(place))#change the locked attribute
        elif not self.place.exits[place][0].locked:
