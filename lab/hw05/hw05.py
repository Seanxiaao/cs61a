...
def cumulative_sum(t):
    ...
    def helper(t):
            if t.is_leaf() is True:
                ...
                return t.label+sum([helper(x) for x in t.children])
    if t.is_leaf() is True:
        return t
    else:
        return Tree(helper(t),[cumulative_sum(x) for x in t.children])
...
def is_valid_bst(bst):
    ...
    if bst == BinTree.empty:
        return True
    if bst.left != bst.empty and bst.right != bst.empty:
            if bst_max(bst.left) > bst.label or bst_min(bst.right) < bst.label:
                return False
            else:
                return is_valid_bst(bst.right) and is_valid_bst(bst.left)
    elif bst.left == bst.empty and bst.right != bst.empty:
        if bst_min(bst.right) < bst.label:
            return False
        else:
            return is_valid_bst(bst.right)
    elif bst.right == bst.empty and bst.left != bst.empty:
        if bst_max(bst.left) > bst.label:
            return False
        else:
            return is_valid_bst(bst.left)
    else:
        return True
...
def add_up(n, lst):
    ...
    True
    """
    sortlist,i  = list(set(lst)), 0
    j = len(sortlist)-1
    while i!=j:
        if sortlist[i] + sortlist[j]==n:
            return True
        elif sortlist[i] + sortlist[j]>n:
            j += -1
        else:
             i += 1
    return False
...
def missing_val(lst0, lst1):
    ...
    True
    """
    set0, set1 = set(lst0), set(lst1)
    return list(set0 - set1)[0]
