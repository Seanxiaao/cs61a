...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        #new operand = [operand.eval(env) for operand in self.operands]
        #then use the apply method to get the number
        return global_env[str(self.operator)].apply([operand.eval(env) for operand in self.operands])
